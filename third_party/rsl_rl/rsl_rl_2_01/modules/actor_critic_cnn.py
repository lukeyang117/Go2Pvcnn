...
from rsl_rl_2_01.networks import CNN, MLP, EmpiricalNormalization
...
class ActorCriticCNN(ActorCritic):
    ...
    def act(self, obs: TensorDict, **kwargs: dict[str, Any]) -> torch.Tensor:
        ...
        if torch.any(~torch.isfinite(mlp_obs)):
            print(f"[ERROR] Non-finite mlp_obs detected before normalization!")
            print(f"  mlp_obs has NaN: {torch.any(torch.isnan(mlp_obs))}, has Inf: {torch.any(torch.isinf(mlp_obs))}")
        
        mlp_obs = self.actor_obs_normalizer(mlp_obs)
        
        # Debug: check after normalization
        if torch.any(~torch.isfinite(mlp_obs)):
            print(f"[ERROR] Non-finite mlp_obs detected after normalization!")
            print(f"  mlp_obs has NaN: {torch.any(torch.isnan(mlp_obs))}, has Inf: {torch.any(torch.isinf(mlp_obs))}")
        ...
        if self.distribution is not None:
            std = self.distribution.scale
            mean = self.distribution.loc
            
            if torch.any(~torch.isfinite(mean)):
                print(f"[ERROR] Non-finite mean detected!")
                print(f"  mean has NaN: {torch.any(torch.isnan(mean))}, has Inf: {torch.any(torch.isinf(mean))}")
                print(f"  mean min: {mean[torch.isfinite(mean)].min().item() if torch.any(torch.isfinite(mean)) else 'all NaN'}")
                
            if torch.any(std < 0):
                print(f"[ERROR] Negative std detected!")
                print(f"  std min: {std.min().item()}, max: {std.max().item()}")
                
            if torch.any(~torch.isfinite(std)):
                print(f"[ERROR] Non-finite std detected!")
                print(f"  std has NaN: {torch.any(torch.isnan(std))}, has Inf: {torch.any(torch.isinf(std))}")
                if hasattr(self, 'log_std'):
                    print(f"  log_std: {self.log_std}")
                    print(f"  log_std has NaN: {torch.any(torch.isnan(self.log_std))}")
                elif hasattr(self, 'std'):
                    print(f"  self.std: {self.std}")
                    print(f"  self.std has NaN: {torch.any(torch.isnan(self.std))}")
    ...
    def get_actor_obs(self, obs: TensorDict) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        obs_list_1d = []
        for obs_group in self.actor_obs_groups_1d:
            obs_data = obs[obs_group]
            # Debug: check for NaN in each observation group
            if torch.any(~torch.isfinite(obs_data)):
                print(f"[ERROR] Non-finite data in observation group '{obs_group}'!")
                print(f"  Shape: {obs_data.shape}")
                print(f"  Has NaN: {torch.any(torch.isnan(obs_data))}, Has Inf: {torch.any(torch.isinf(obs_data))}")
                print(f"  Sample values: {obs_data[0] if obs_data.numel() > 0 else 'empty'}")
            obs_list_1d.append(obs_data)
        
        obs_dict_2d = {}
        for obs_group in self.actor_obs_groups_2d:
            obs_dict_2d[obs_group] = obs[obs_group]
        return torch.cat(obs_list_1d, dim=-1) if obs_list_1d else torch.tensor([], device=obs.device), obs_dict_2d
