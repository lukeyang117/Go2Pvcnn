"""Teacher elevation trajectory env: high-res height scan + downsampled observation + reference reward slots."""
...
from extension.mdp.reference_trajectory_events import replan_raw_reference_trajectories
...
class TeacherElevationTrajectoryRewardsCfg(BaseRewardsCfg):
    """Base locomotion rewards plus placeholder reference reward slots."""
...
class TeacherElevationTrajectoryEnvCfg(TeacherElevationEnvCfg):
    """Trajectory-guided teacher env built on top of teacher_elevation."""
    ...
    use_raw_reference_trajectory: bool = False
    reference_trajectory_horizon: int = 50
    #: Wall-clock replan period in simulation seconds (global for all envs when raw reference is enabled).
    reference_replan_interval_s: float = 5.0
    #: ``process`` (default): ``spawn`` multiprocessing pool, true CPU parallelism. ``thread``: thread pool (GIL-bound).
    raw_reference_parallel_backend: str = "process"
    #: Max concurrent raw planners; ``0`` means ``min(num_envs, cpu_count)`` (no artificial 32 cap).
    raw_reference_num_threads: int = 0

    def __post_init__(self):
        super().__post_init__()
        if self.scene.height_scanner is not None:
            self.scene.height_scanner.update_period = self.decimation * self.sim.dt
        iv = float(self.reference_replan_interval_s)
        self.events.startup_raw_reference = EventTerm(
            func=replan_raw_reference_trajectories,
            mode="startup",
            params={
        ...
        self.events.interval_raw_reference = EventTerm(
            func=replan_raw_reference_trajectories,
            mode="interval",
            interval_range_s=(iv, iv),
            is_global_time=True,
            params={
                "sensor_cfg": SceneEntityCfg("height_scanner"),
                "asset_cfg": SceneEntityCfg("robot"),
...
class TeacherElevationTrajectoryEnvCfg_PLAY(TeacherWithoutSemanticEnvCfg_PLAY):
    """Play config for trajectory-guided elevation teacher."""
