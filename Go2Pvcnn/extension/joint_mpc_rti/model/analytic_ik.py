...
def go2_analytic_ik_selected(
    root_pos_w: Tensor,
    root_rpy_w: Tensor,
    foot_target_w: Tensor,
    leg_index: Tensor,
) -> tuple[Tensor, Tensor]:
    """Return analytic IK for one selected leg per leading element."""
    root_pos = torch.as_tensor(root_pos_w)
    root_rpy = torch.as_tensor(
        root_rpy_w, dtype=root_pos.dtype, device=root_pos.device
    )
    foot_target = torch.as_tensor(
        foot_target_w, dtype=root_pos.dtype, device=root_pos.device
    )
    index = torch.as_tensor(leg_index, dtype=torch.long, device=root_pos.device)
    if root_pos.ndim < 2 or root_pos.shape[-1] != 3:
        raise ValueError("root_pos_w must have shape [...,3]")
    if root_rpy.shape != root_pos.shape or foot_target.shape != root_pos.shape:
        raise ValueError("root_rpy_w and foot_target_w must match root_pos_w")
    if index.shape != root_pos.shape[:-1]:
        raise ValueError("leg_index must match the leading root shape")

    rotation_wb = rpy_to_rotation_matrix(root_rpy)
    target_delta_w = foot_target - root_pos
    target_b = torch.einsum(
        "...ji,...i->...j", rotation_wb.transpose(-1, -2), target_delta_w
    )
    flat_index = index.reshape(-1)
    hip = torch.index_select(
        constant_like(root_pos, "selected_ik_hip_offsets", HIP_OFFSETS),
        0,
        flat_index,
    ).reshape(*index.shape, 3)
    side = torch.index_select(
        constant_like(root_pos, "selected_ik_leg_side_signs", LEG_SIDE_SIGNS),
        0,
        flat_index,
    ).reshape(index.shape)
    hip_local = target_b - hip
    ...
    thigh = float(THIGH_LENGTH)
    calf = float(CALF_LENGTH)
    cosine_calf = (reach_sq - thigh * thigh - calf * calf) / (
        2.0 * thigh * calf
    )
    calf_angle = -torch.acos(cosine_calf.clamp(-1.0, 1.0))
    alpha = torch.atan2(-px, -effective_z)
    beta = torch.atan2(
        calf * torch.sin(calf_angle), thigh + calf * torch.cos(calf_angle)
    )
    thigh_angle = alpha - beta
    joint = torch.stack((abad, thigh_angle, calf_angle), dim=-1)
    tolerance = 1.0e-6
...
__all__ = ["go2_analytic_ik", "go2_analytic_ik_selected"]
