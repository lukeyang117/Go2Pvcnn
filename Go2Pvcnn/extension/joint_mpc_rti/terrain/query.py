...
def query_perceptive_world(
    ...
    return JointMpcPerceptiveQuery(
        ...
        ground_support_height_w=_bilinear(
            field.ground_support_height_w, index_x, index_y
        ),
...
def query_ground_support_height_world(
    field: JointMpcPerceptiveField,
    points_w: Tensor,
) -> Tensor:
    """Query root support ground without materializing unrelated field channels."""
    points = torch.as_tensor(
        points_w, dtype=field.height_w.dtype, device=field.height_w.device
    )
    if points.ndim != 3 or int(points.shape[-1]) not in (2, 3):
    ...
    return _bilinear(field.ground_support_height_w, index_x, index_y)
...
__all__ = [
    ...
    "query_ground_support_height_world",
