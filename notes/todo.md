...
- Current focus: **T302p MPC command-frame alignment / flat all-direction direction metrics**.
- Active branch page: [T302p](todo/T302p-mpc-command-frame-alignment-plan.md).
- Active implementation plan: [T302p MPC command frame alignment plan](todo/T302p-mpc-command-frame-alignment-plan.md).
- Active code surface:
...
- Current contract:
  ...
  - T302p remaining acceptance: run the full eight-command flat direction set with enough steps for moving-leg metrics, and rerun low-small semantic compatibility. The short smoke had command match error `0.0`, root cosine `0.998857`, root lateral ratio `0.047792`, but one leg lateral ratio was `0.149278 > 0.10`.
  - T302p hard constraint: coordinate-frame alignment only. Do not add losses, delete losses, change loss weights, change optimizer behavior, add hard projection, or postprocess snapping.
...
| --- | --- | --- | --- |
| T302p | active | Command-frame implementation is local and guarded; short real smoke confirms command-source equality and root direction, but full all-direction/per-leg and semantic compatibility acceptance remain open. | Run [T302p](todo/T302p-mpc-command-frame-alignment-plan.md) Task 7 Step 2 eight-command flat set, then Task 8 low-small semantic compatibility regression. |
...
| --- | --- | --- | --- | --- | --- |
| T302p | active | MPC command-frame alignment | [T302p](todo/T302p-mpc-command-frame-alignment-plan.md) | Implementation local; focused `184 passed`; fixed-forward 5-step smoke passes root direction but one leg lateral ratio exceeds `0.10`; eight-command and low-small semantic regression remain. | design [2026-06-06](../docs/superpowers/specs/2026-06-06-mpc-command-frame-alignment-design.html); implementation [2026-06-06 18:58](log/2026-06-06-1858-t302p-command-frame-implementation.md) |
...
| T302p.2 | T302p | active | P0 | Full acceptance remains open: eight flat commands need longer real smoke, moving-leg XY direction must pass when motion is sufficient, and low-small semantic compatibility must be rerun. | [T302p plan](todo/T302p-mpc-command-frame-alignment-plan.md#task-7-real-isaaclab-flat-all-direction-smoke) |
