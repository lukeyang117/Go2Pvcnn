...
- Current focus: T116 `K=5` mode-first small-obstacle crossing has now closed the `T116i` follow-up for nonzero speed candidate tables and hard-reason diagnostics.
  ...
  - New T300f swing trajectory quality probe reproduces the user screenshot as numeric MPC decoded-foot trajectory anomalies on task terrain: `max_worst_max_to_median_step=15.795617`, `max_worst_boundary_to_median_step=10.772456`, and `min_z_quadratic_r2=0.300820`. Decode tracing narrows first failure to optimizer-updated foot residuals: initial decode stays smooth (`R2≈0.994`) while optimized pre-lock decode drops to `R2=0.163466/0.206992` for forward/yaw. A 300-step cobblestone long-horizon sweep covers all `7` subterrain types x `9` yaw-heavy speed commands; worst terrain is `boxes score_mean=44.013797`, worst case is `boxes vx=0.50 vy=0.25 yaw=1.00 score=76.440777`. User accepted production `boundary8+accel8` only, rejecting residual/parabola; backend `99 passed`, and focused worst-case repro improves to `score=20.870477`, `jump=4.720598`, `boundary=8.645891`, `R2=0.530423`.
...
| T300f | User-reported MPC viewer swing trajectory discontinuity is reproduced with numeric metrics; production now adds boundary continuity + second-difference foot acceleration regularization only, leaving nominal residual and parabola constraints out by user decision. | Broaden 300-step cobblestone and T302 collision/obstacle regressions before closing the trajectory-quality branch. |
...
| T301a/T301b | Viewer `R` reset semantics now preserve current root `xy/yaw`, restore standing joints, and ground feet from scanner terrain. Step-mode adds explicit Space single-frame viewer/play controls, keeps IsaacLab/Kit window render pump alive while paused, aligns trajectory marker updates to Space, and defers viewer movement commands to the next trajectory boundary. | Optional manual livestream acceptance for step-mode feel; add a focused IsaacLab headless reset check later without disturbing the active T300e branch. |
...
| T301 | verify | viewer interaction / grounded reset behavior + step-mode playback | [T301](todo/T301-viewer-r-key-grounded-reset.md) | `R` reset helper now preserves current root `xy/yaw`, restores initial standing joints, clears command buffer, and grounds feet from scanner terrain. `T301b` adds explicit viewer/play `--step-mode`, Space single-frame machine/marker stepping, window render pump while paused, and viewer command deferral until trajectory end. | feature `pending`; verified `local+env_isaacsim test_viewer_reset + py_compile + diff check + headless viewer step-mode smoke` |
...
| T301b | T301 | verify | P1 | Explicit viewer/play step-mode is implemented and headless-smoked; paused loop keeps IsaacLab/Kit rendering, marker updates align with Space, remaining risk is manual livestream feel for terminal Space and mid-trajectory command deferral. | [T301 branch](todo/T301-viewer-r-key-grounded-reset.md#t301b-step-mode空格单帧播放与轨迹边界命令切换) + [log](log/2026-05-22-0004-t301b-step-mode-viewer-play.md) |
...
| --- | --- | --- | --- | --- |
| 2026-05-22 13:58 | MPC swing trajectory quality reproduction | pass as reproduction/root-cause trace; optimized `foot_pos_residual` first breaks swing shape; test-only `smooth24` sweep wins so far | [T300](todo/T300-unified-dense-mpc-backend.md) | [2026-05-22-1358-mpc-swing-trajectory-quality-reproduction.md](log/2026-05-22-1358-mpc-swing-trajectory-quality-reproduction.md) |
| 2026-05-22 00:04 | T301b step-mode viewer/play | pass with scoped verification; viewer/play `--step-mode` implemented, paused loop keeps render pump alive, Space advances machine/marker step | [T301](todo/T301-viewer-r-key-grounded-reset.md) | [2026-05-22-0004-t301b-step-mode-viewer-play.md](log/2026-05-22-0004-t301b-step-mode-viewer-play.md) |
