...
| T300d | Subagent-driven MPC implementation and runtime counters are integrated; drift/touchdown probes have now been synthesized into production `dir15 + dir19` foothold memory, yaw-entry ramp, and terrain-grounded contact foot z. | User visual inspection is the next acceptance step; if artifacts remain, continue from terrain-height-in-loss and IK/contact smoothness rather than another pure same-matrix drift sweep. |
...
| T300 | verify | unified dense MPC backend design and implementation | [T300](todo/T300-unified-dense-mpc-backend.md) | `batch_mpc_planner` integration + focused verification complete; long replan drift/touchdown probes have been synthesized into production foothold memory + terrain grounding | feature `pending`; verified `focused mpc backend tests + py_compile + IsaacLab smoke + drift/touchdown probes` |
...
| T300d | T300 | verify | P0 | Subagent implementation/testing is merged; long replan drift and touchdown-grounding probes have been synthesized into production `dir15 + dir19` foothold memory, yaw-entry ramp, and terrain-grounded contact z; 4096 runtime counter extraction is still blocked by Isaac high-scale instability. | [T300 branch node](todo/T300-unified-dense-mpc-backend.md#t300d-subagent-driven-implementation-and-test-execution-for-extensionbatch_mpc_planner) + [drift log](log/2026-05-12-2147-mpc-long-replan-foot-drift-reproduction.md) + [mixed/sequence log](log/2026-05-13-0932-mpc-mixed-sequence-long-horizon-sweep.md) + [touchdown probe log](log/2026-05-13-1025-mpc-touchdown-grounding-probe.md) + [production grounding log](log/2026-05-13-1253-mpc-dir15-dir19-production-grounding.md) |
