...
- T302o Task 3 rollout skeleton is implemented and verified: static output/policy-loading contracts pass, pycompile and diff check exit `0`, and real IsaacLab tracking smoke exits `0` with `config.json`, `metrics.jsonl`, `rounds.jsonl`, and `summary.json` under `logs/mpc_policy_eval/task3_smoke/2026-06-05_17-00-41`; Task 4 tracking metrics next.
...
| T302o.1 | T302o | active | P0 | Task 3 rollout skeleton and output files are verified with real IsaacLab smoke; next is Task 4 tracking mode runtime metrics. | [T302o plan](todo/T302o-mpc-policy-eval-plan.md#task-4-tracking-mode-runtime-metrics) |
