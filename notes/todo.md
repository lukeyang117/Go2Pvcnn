...
- T302o Task 4 tracking runtime metrics are implemented locally: per-step `metrics.jsonl`, per-round `rounds.jsonl`, and top-level `summary.json` include tracking aggregates; reference feet use `_trajectory_manager.current_reference()["foot_pos_w"]` with `_trajectory_reference_cache` + `current_frame_ids()` fallback; local metric/static tests and pycompile pass; real IsaacLab smoke remains for main-agent verification.
...
| T302o | active | New evaluation route will load policy checkpoints while enabling MPC reference/cache, report tracking metrics, and measure flat small-obstacle collision rate by collided envs per round. | Task 4 local tracking metrics are implemented; run main-agent real IsaacLab tracking smoke, then implement Task 5 small_collision runtime metrics. |
...
| T302o.1 | T302o | active | P0 | Task 4 tracking runtime metrics are implemented and locally verified; real IsaacLab tracking smoke and Task 5 small_collision runtime metrics remain. | [T302o plan](todo/T302o-mpc-policy-eval-plan.md#task-5-small-collision-mode-runtime-metrics) |
...
| --- | --- | --- | --- | --- |
| 2026-06-05 | T302o Task 4 tracking runtime metrics | pass locally; RED missing `TrackingRoundAccumulator`; GREEN metric `6 passed`, static `6 passed`, pycompile exit `0`; real IsaacLab smoke not run by implementer per delegation | [T302o](todo/T302o-mpc-policy-eval-plan.md) | [2026-06-05-t302o-task4-tracking-runtime-metrics.md](log/2026-06-05-t302o-task4-tracking-runtime-metrics.md) |
