...
- Current focus: T116 `K=5` mode-first small-obstacle crossing has now closed the `T116i` follow-up for nonzero speed candidate tables and hard-reason diagnostics.
  ...
  - New T302 child T302g is now the design/plan front for an independent MPC semantic RL train/play config: high-resolution `semantic_height_scanner`, `2 x 16 x 16` height+priority-semantic CNN input, foot-only MPC imitation reward, IsaacLab-body-buffer swing collision reward, dirty-subset 4096 replanning, and a `10s` collect-data timing gate without T302 metric regression.
- Read next:
  ...
  - [T302g plan](../docs/superpowers/plans/2026-05-18-mpc-semantic-rl-training-config.md)
...
| T302g | todo | MPC semantic RL train/play rollout config | [T302g](todo/T302g-mpc-semantic-rl-training-config.md) | design and implementation plan written for independent MPC semantic RL config with 4096 collect-data timing gate and T302 non-regression gate | feature `pending`; verified `design/plan only` |
...
| T302g.1 | T302g/T302 | todo | P0 | Independent MPC semantic RL train/play config is designed and planned; implementation must add new config file, semantic scanner observation, foot-only imitation, swing collision reward, dirty-subset 4096 timing under `10s`, and T302 non-regression evidence. | [T302g branch](todo/T302g-mpc-semantic-rl-training-config.md) + [plan](../docs/superpowers/plans/2026-05-18-mpc-semantic-rl-training-config.md) |
