...
| T300f | User-reported MPC viewer swing trajectory discontinuity is reproduced with numeric metrics; decode tracing shows optimizer-updated `foot_pos_residual` first breaks the smooth/parabolic shape, and test-only sweep selects `smooth24` as the best direction so far. | Keep production code unchanged until adding a backend quality test; then evaluate a minimal swing-shape regularizer against T302 collision/obstacle regressions. |
