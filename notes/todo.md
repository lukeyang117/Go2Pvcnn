...
- T302o design is approved enough for implementation planning: [../docs/superpowers/specs/2026-06-05-mpc-policy-eval-design.html](../docs/superpowers/specs/2026-06-05-mpc-policy-eval-design.html). It adds one Python entry under `Go2Pvcnn/scripts/`, not a shell script, and keeps `scripts/play.py` no-MPC behavior unchanged.
  ...
  - T302o Task 5 small_collision runtime metrics are locally verified: per-round `rounds.jsonl` reports `collided_env_count`, env-rate, first collision steps, body names, and max small contact force from `semantic_contact_small.data.force_matrix_w`; top-level `summary.json` aggregates by `total_collided_envs / total_env_rounds`.
...
| T302o | active | New evaluation route will load policy checkpoints while enabling MPC reference/cache, report tracking metrics, and measure flat small-obstacle collision rate by collided envs per round. | Task 5 small_collision metrics are locally verified; run main-agent card0/env_isaacsim small_collision smoke, then implement Task 6 livestream markers. |
...
| T302o.1 | T302o | active | P0 | Task 5 small_collision runtime metrics are implemented locally; real small_collision smoke and Task 6 livestream markers remain. | [T302o plan](todo/T302o-mpc-policy-eval-plan.md#task-6-livestream-command-sync-and-mpc-foot-markers) |
...
| --- | --- | --- | --- | --- |
| 2026-06-05 | T302o Task 5 small_collision runtime metrics | pass locally; metric `7 passed in 1.61s`, static `7 passed in 0.03s`, pycompile exit `0`, diff check exit `0`; real IsaacLab smoke not run by implementer | [T302o](todo/T302o-mpc-policy-eval-plan.md) | [2026-06-05-t302o-task5-small-collision-runtime-metrics.md](log/2026-06-05-t302o-task5-small-collision-runtime-metrics.md) |
